...
def main():
    ...
    for num_feature in range(1, 7):
        pca_train, pca_train_arr = run_PCA(X.transpose(), num_feature)

        svc = run_linear_SVM(X_train, y_train, C)
        rbf_svc = run_rbf_SVM(X_train, y_train, C)

        pca_test, pca_test_arr = run_PCA(X.transpose(), num_feature)

        print("%d:\t%f\t%f" % (num_feature,
                                   test_svm_models(X_test, y_test, svc),
                                   test_svm_models(X_test, y_test, rbf_svc)))
...
def load_data():
    ...
    class_df = np.array(wine_data.pop('class'))
    feature_df = np.array(wine_data)
